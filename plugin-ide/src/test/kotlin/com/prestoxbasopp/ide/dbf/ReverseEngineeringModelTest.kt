...
class ReverseEngineeringModelTest {
    ...
    fun `generate produces read-only and relational method surfaces respecting aliases and table overrides`() {
        ...
        assertThat(dog.methods.map { it.methodName }).contains("insert", "update", "upsert", "delete", "commit", "getOwner", "addOwner", "removeOwner")
        assertThat(dog.methods.mapNotNull { it.alias }).contains("l", "f", "i", "u", "us", "d", "c")
        ...
        assertThat(dog.source).contains("CLASS METHOD Dog:commit(options)")
        assertThat(dog.source).contains("CLASS METHOD Dog:openRepository(options)")
        assertThat(dog.source).contains("CLASS METHOD Dog:openDbManager(options)")
        assertThat(dog.source).contains("CLASS DaoDbManager")
        assertThat(dog.source).contains("METHOD DaoDbManager:commit()")
        assertThat(dog.source).contains("RETURN repo:commit()")
    ...
    fun `generate emits production persistence details including coercion and repository resolution`() {
        ...
        assertThat(artifact.source).contains("RETURN DaoDbManager():init(DaoRepositoryProvider():default())")
        assertThat(artifact.source).contains("LOCAL manager := iif(ValType(options) == \"H\" .AND. HHasKey(options, \"dbManager\"), options[\"dbManager\"], NIL)")
    ...
    fun `generate persistence methods avoid object key hash misuse and support hash key extraction`() {
        ...
        assertThat(artifact.source).contains("CLASS METHOD Dbasef5:commit(options)")
        assertThat(artifact.source).contains("RETURN manager:commit()")
    ...
    fun `generate skips blank class names and supports disabled aliases`() {
        ...
        assertThat(artifacts.single().source).contains("CLASS METHOD commit(...)")
